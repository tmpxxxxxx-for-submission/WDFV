...
class DDFF12Loader(Dataset):
    ...
    def __init__(self, hdf5_filename,  stack_key="stack_train", disp_key="disp_train", transform=None,
        ...
        self.max_n_stack = 10
        self.b_test = b_test
        self.dataset_scale = 2
        with h5py.File(hdf5_filename, 'r') as file:
            self.dataset_len = file[self.stack_key].shape[0]
        if 'train' in self.stack_key:
            self.dataset_len *= self.dataset_scale
        ...
        if transform is None:
            if 'train' in self.stack_key:
                self.transform = self.__create_preprocessing(
                    crop_size=(224, 224), cliprange=None, b_filp=True)
            else:
    ...
    def __getitem__(self, idx):
        ...
        if 'train' in self.stack_key:
            sub_idx = idx % self.dataset_scale
            idx = idx // self.dataset_scale
        ...
        try:
            if 'test' in self.stack_key:
                ...
                sample = {'input': self.hdf5[self.stack_key][idx].astype(
                    float), 'output': self.hdf5[self.disp_key][idx], 'sub_idx': sub_idx}
        except:
            sample = None
            for _ in range(100):
                sample = {'input': self.hdf5[self.stack_key][idx].astype(
        ...
        if self.n_stack < self.max_n_stack:
            if 'train' in self.disp_key:
                ...
                rand_idx = np.linspace(0, 9, self.n_stack, endpoint=True)
            # rand_idx = np.linspace(0, 9, self.n_stack, endpoint=True)
    ...
    def __create_preprocessing(self, crop_size=None, cliprange=[0.0202, 0.2825], mean=[0.485, 0.456, 0.406],
        ...
        # transform += [self.RandomInvert()]
        # transform += [self.RandomRotate()]
    ...
    class ToTensor(object):
        ...
        def __call__(self, sample):
            ...
            sample['input'] = torch.from_numpy(sample['input'].transpose(
                (0, 3, 1, 2))).float().div(255)  # I add div 255
        ...
        def __call__(self, sample):
            ...
            if self.mean_output is None or self.std_output is None:
                ...
                output_image = torchvision.transforms.functional.normalize(sample['output'], mean=self.mean_output,
                                                                           std=self.std_output)
            sample['input'] = input_images
            sample['output'] = output_image
            return sample
    ...
    class RandomFilp(object):
        ...
        def __call__(self, sample):
            ...
            sample['input'] = np.ascontiguousarray(inputs)
            sample['output'] = np.ascontiguousarray(target)
            return sample
    ...
    class RandomInvert(object):
        ...
        def __init__(self):
            self.inverter = torchvision.transforms.RandomInvert()

        def __call__(self, sample):
            inputs = sample['input']

            inputs = self.inverter(inputs)
    ...
    class RandomRotate(object):
        ...
        def __init__(self, deg_range=180):
            self.rotater = torchvision.transforms.functional.rotate
            self.inter_mode = torchvision.transforms.InterpolationMode.BILINEAR
            self.deg_range = deg_range

        def __call__(self, sample):
            inputs, target = sample['input'], sample['output']

            cur_deg = torch.rand(1).item() * \
                (self.deg_range * 2) - self.deg_range
            inputs = self.rotater(
                inputs, cur_deg, interpolation=self.inter_mode)
            target = self.rotater(
                target, cur_deg, interpolation=self.inter_mode)
    ...
    class RandomCrop(object):
        def __init__(self, output_size, valid_crop_threshold=0.8):
            ...
            self.crop_hw = [
                (0, 0), (0, 107), (0, 214), (0, 328),
                (159, 0), (159, 107), (159, 214), (159, 328),
            ]
        ...
        def __call__(self, sample):
            ...
            # if sub_idx < len(self.crop_hw):
            #     (y, x) = self.crop_hw[sub_idx]
            #     output_image = sample['output'][:,
            #                                     y:(y + new_h), x:(x + new_w)]
            ...
            if sub_idx == 0:
                candidates = np.asarray(
                    [(x, y) for y in range(0, 20, 2) for x in range(0, w - new_w, 2)])
            ...
            if sub_idx == 1:
                candidates = np.asarray(
                    [(x, y) for y in range(139, 159, 2) for x in range(0, w - new_w, 2)])
                np.random.shuffle(candidates)
                for x, y in candidates:
                    output_image = sample['output'][:,
                                                    y:(y + new_h), x:(x + new_w)]
            ...
            candidates = np.asarray(
                [(x, y) for y in range(0, h - new_h, 2) for x in range(0, w - new_w, 2)])
            np.random.shuffle(candidates)
